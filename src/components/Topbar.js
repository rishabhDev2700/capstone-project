...
export const Topbar = () => {
  ...
  const handleForm = (e) => {
    ...
    const data = {
      ...
      date: target.dateAdded.value,
    };
    const temp = [...todo.todoList, data];
  ...
  return (
    <header className="w-full bg-violet-700 text-white flex justify-evenly items-center p-4 shadow-sm shadow-purple-800 sticky top-0">
      ...
      >
        <form
          className="flex flex-col p-4 bg-white text-black border-2 border-violet-400 rounded-md shadow-lg shadow-blue-900"
        ...
        >
          ...
          ></textarea>
          <input
            className="bg-violet-500 text-white border-2 border-violet-500 hover:bg-white hover:text-violet-500 p-4 m-4 ease-in-out duration-300"
            type="submit"
            value="Create"
          />
        </form>
